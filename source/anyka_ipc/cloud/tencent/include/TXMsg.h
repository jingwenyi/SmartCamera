...
/**
 * 与 tag_structuring_msg 区别
 * 文件支持本地路径或URL
 * file_path_url 仅图片消息支持URL，其他消息的URL，手Q端暂未支持
 * thumb_path_url 图片消息 & 视频消息的缩略图，支持URL
 * 增加jump_url，结构化消息支持点击跳转
 */

typedef struct tag_structuring_msg2
{
    // char * '\0'结尾UTF-8字符串
    int                     msg_id;
    char*                   file_path_url;      // 如果是文件路径，则会上传，如果是URL，则会透传到App端 仅图片消息支持URL，其他消息的URL，手Q端暂未支持
    char*                   thumb_path_url;     // 如果是文件路径，则会上传，如果是URL，则会透传到App端 图片消息 & 视频消息的缩略图，支持URL
    char*                   title;
    char*                   digest;
    char*                   guide_words;
    ...
    char*                   jump_url;           // 跳转URL，点击结构化消息会跳转到此链接
} structuring_msg2;

/**
 * 接口说明：向手机QQ发送结构化消息，每条消息都会有唯一的 cookie, tx_send_msg_notify 用于了解发送状态
 * 与tx_send_structuring_msg的区别：structuring_msg2的文件支持URL透传到App端
 * 支持跳转链接 jump_url
 * msg：结构化消息
 * notify：进度，结果回调
 * cookie：返回任务cookie
 */
SDK_API void tx_send_structuring_msg2(const structuring_msg2 *msg, tx_send_msg_notify *notify, unsigned int *cookie);
