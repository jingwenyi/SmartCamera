...
static pthread_mutex_t     g_config_lock;
...
{	
    ...
    if(file_id == NULL)
    {
        anyka_print("[%s,%d]:config file didn't open : %s\n",__func__, __LINE__, config_name);
...
* @param[in]  void *config_handle 配置文件句柄
                     ...
                     char *name 配置项名称
                     char *value 要设置的值
                     char *default_value   配置项的默认值，如果发现没有，将给这个值
* @return int
* @retval if return 0 success, otherwise failed 
*/
...
{    
    ...
    if(config->update)
    {
        ...
        while(config)
        {
			...
			size += strlen(config->title) + 4;
            item = config->item;
            while(item)
            {
                sprintf(file_buf,"%s\t\t\t= %s\n", item->name, item->value);
                fputs(file_buf, file_id);
				size += strlen(item->name) + strlen(item->value) + 6;
                item = item->next;
            }
			config = config->next;
        }        
	    ...
	    if(stat(config_name, &statbuff) < 0)
		{    
	        filelen = 0;    
	    }
		else
		...
		//anyka_print("[%s,%d]:anyka_config_save, size = %d, filelen = %d\n",__func__, __LINE__, size, filelen);
...
{
	pthread_mutex_init(&g_config_lock, NULL);
}
